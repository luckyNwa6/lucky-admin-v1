...
<script>
// import { genToken } from '@/api/login' //这是本地测试登录的
import { genToken } from '@/api/loginIdp' //线上认证服务器的登录

import { getQQ, getPersonInfo } from '@/api/loginIdp'
  ...
  methods: {
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          if (this.yzmInput.toLowerCase() !== this.yzmm.toLowerCase()) {
            ...
            //下面是登录逻辑 第一个是登录资源服务器里直接写死，第二个去认证服务器
            //1
            // genToken(this.$qs.stringify({ acc: this.form.acc, pwd: this.form.pwd })).then((res) => {
            //2
            let data1 = {
    ...
    goQQ() {
      getQQ().then((res) => {
        ...
        // window.open(res, 'QQ验证', windowFeatures)
