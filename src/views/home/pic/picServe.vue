...
<script>
import { getYunList, delRemotePic, modifyInfo, getFolderList } from '@/api/pic.js'
export default {
  data() {
    return {
      tableData: [],
  ...
  methods: {
    ...
    confirmDialog() {
      // 处理弹框确认逻辑
      modifyInfo({ id: this.tempId, picName: this.picName, folder: this.selectedValue }).then((res) => {
  ...
  created() {
    this.getYunList()
    getFolderList({ folderName: '', userId: JSON.parse(this.$cookie.get('picData')).userId }).then((res) => {
